...
| **Third-party duplex** | FDB / τ-Voice / others | FDB v2–v3, τ-Voice | Comparable floor + tool numbers, if the API was actually called |
| **Third-party adjacent** | Scale, VoiceBench authors, Coval | Audio MultiChallenge, VoiceBench, WildSpeech | Content / latency. Do not mix with [Interaction](interaction.md) |
...
Third parties also run Realtime on [FDB v2](../README.md#3-multi-turn-content), [FDB v3](../README.md#4-task-and-tools), [τ-Voice](../README.md#4-task-and-tools), and [Audio MultiChallenge](../README.md#3-multi-turn-content).
...
| They report | Class here | Notes | Source |
|:--|:--|:--|:--|
| **Big Bench Audio** | Adjacent (audio reasoning) | Claimed #1, independently checked by Artificial Analysis | [Grok Voice Agent API](https://x.ai/news/grok-voice-agent-api) |
...
Third parties also run Grok Voice on FDB v3 and τ-Voice. Not in the Audio MultiChallenge paper table.
...
The tech report does **not** headline FDB or τ-Voice. Audio MultiChallenge does include Qwen3-Omni (audio out, 24.34% pass).
...
\* VoiceBench tables usually list GPT-4o-Audio, not always the later Realtime SKU.

If you only need one public stack that hits GPT, Gemini, and Grok on **floor + tools**, use FDB v3 and τ-Voice. Add Audio MultiChallenge for multi-turn content. Do not fill Seed / Qwen gaps by averaging VoiceBench or MOS into Interaction.

## Do not mix

1. Big Bench Audio / VoiceBench / MMAU ≠ [Interaction](interaction.md) TOR.
