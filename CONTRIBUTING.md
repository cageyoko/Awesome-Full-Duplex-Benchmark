...
`Component` and `Challenge` are tags (Granularity / Protocol), not sections. The same paper may appear in more than one class. Speech-in QA or half-duplex tool benches go under **Adjacent**, not a sixth class.
...
2. Put the row in the class that matches **what is scored**, not the venue or the GitHub repo. Duplicate the row if it is first-class in a second class (for example FDB v1 under Interaction and Timing; τ-Voice under Task and Interaction).
